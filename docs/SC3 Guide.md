...
- [Binary Streams](#binary-streams)
  - [SubstreamDuplex API](#substreamduplex-api)
  - [Node: opening a stream and wiring pumps](#node-opening-a-stream-and-wiring-pumps)
  - [Channel: bridging Node streams to WebSockets](#channel-bridging-node-streams-to-websockets)
  - [Channel: creating streams directly](#channel-creating-streams-directly)
  - [Browser: attaching to a stream](#browser-attaching-to-a-stream)
  - [CachedData integration: reacting to pipeline events](#cacheddata-integration-reacting-to-pipeline-events)
  - [Stream lifecycle cleanup](#stream-lifecycle-cleanup)
  - [Multiplexing and the binary frame protocol](#multiplexing-and-the-binary-frame-protocol)
...
Binary streams provide named, multiplexed, bidirectional byte pipes across the full SC3 stack. They use native binary frames on both network hops (WebSocket) and process boundaries (IPC child processes are forked with `serialization: 'advanced'`, so binary frames cross IPC natively). The application uses string streamIds; the framework handles uid assignment, frame encoding, consumer pump management, and inter-layer relay.

A SubstreamDuplex contains two independent unidirectional streams:

- **Collator** (many→one): `stream.collator` — a `QueueIterator`. Multiple writers, one reader. Used for browser→server input (keystrokes). The application writes to it via `stream.process(frame)` and reads it via `for await (const frame of stream.collator)`.
- **Fanout** (one→many): `stream.fanout` — an `AsyncFanout`. One writer feeds it via `stream.write(frame)`; multiple consumers each get an independent copy. Used for server→browser output (terminal data).

These never cross. Collator data stays in the collator path. Fanout data stays in the fanout path. IPC and WebSocket relays bridge collator-to-collator and fanout-to-fanout at each boundary.

This section covers the application-facing entry point — opening a stream from a Node. For the complete API surface (the `SubstreamDuplex` interface, the `StreamIndex` registry, the uid-addressed frame protocol, the declarative advertisement/subscription model, and the Channel, Passthrough, and browser layer glue), see `SecureChannel/docs/binary streams.md`, which is the authority for stream internals.

### SubstreamDuplex API

`SubstreamDuplex` is exported from `securechannel`. It takes no constructor arguments — the streamId↔uid identity lives in the enclosing `StreamIndex`, not the duplex. Application code at the Node layer obtains an instance from `this.openStream(streamId)` rather than constructing one directly.

```javascript
// Obtained from this.openStream(streamId) at the Node layer:
stream.write(frame);     // push a frame to the fanout (one→many)
stream.process(frame);   // push a frame to the collator (many→one)
stream.collator;         // the collator QueueIterator — read with for-await
stream.fanout;           // the fanout AsyncFanout — each consumer() is independent

// Reading the fanout — each for-await creates an independent consumer:
for await (const frame of stream) { /* ... */ }

// Lifecycle:
stream.close();          // orderly shutdown — ends fanout and collator (alias for end())
stream.fail(reason);     // error shutdown — fails fanout and collator
stream.ended;            // boolean lifecycle flag
```

Frames are built and parsed by the static `BinaryFrame` class (`BinaryFrame.build(frameType, uid, data)` / `BinaryFrame.parse(buffer)`) and addressed by a numeric `uid`. The application never sees uids — it uses string streamIds, and the framework maps streamId↔uid at every link via `StreamIndex`.

### Node: opening a stream and wiring pumps

The Node is the application's entry point into the stream system. `this.openStream(streamId)` creates the `SubstreamDuplex`, mints the stream's uid, wires the fanout pump toward the Worker, and publishes the advertisement upward.

```javascript
class DeploymentNode extends Node {
    async executeSshDeployment() {
        const stream = this.openStream(this.node);  // streamId = node identifier

        // Fanout: SSH stdout → stream.write() → consumer pump → binary IPC → Worker
        const pumpToStream = async (source) => {
            for await (const chunk of source) stream.write(chunk);
        };

        // Collator: Worker → binary IPC → stream.collator → SSH stdin
        const pumpFromBrowser = async () => {
            for await (const frame of stream.collator) {
                if (this.sshConnection && frame) this.sshConnection.writeStdin(frame);
            }
        };
        pumpFromBrowser();

        await this.sshConnection.executeCommand(command, {
            rawStdout: pumpToStream,
            rawStderr: pumpToStream
        });
    }

    async end(code = 0) {
        if (this.sc3StreamId) this.closeStream(this.sc3StreamId);
        // ...
    }
}
```

`this.openStream(streamId)` on a Node:
1. Registers the stream in `this.streamIndex`, minting its own uid.
2. Starts a fanout consumer pump: reads the fanout and sends binary `BinaryFrame` buffers upward over IPC via `sendBinary(BinaryFrame.data, streamId, payload)`.
3. Calls `publishStreams()`, which emits a `streams` advertisement (`{ streams: [{ streamId, uid }] }`) upward.

The Worker's `_NodeEvents.streams` handler reconciles its child uid namespace (`_childStreamIndex`) against the advertised set, opens the corresponding stream on the channel, and re-publishes upward. Inbound binary frames from browsers arrive at the Node via `resolveStream(...).process(payload)` and surface on `stream.collator`.

`this.closeStream(streamId)` tears the stream down and re-publishes the advertisement (now without that stream), and the upstream layers reconcile it away.

### Channel, Passthrough, and browser layers

A Node-originated stream reaches browser clients automatically. The Worker, Channel, Passthrough, and browser layers each own a `StreamIndex` and reconcile against the bottom-up `streams` advertisement, opening and closing local `SubstreamDuplex` instances to match. A browser subscribes to a stream by issuing a `streamSubscribe` command, which causes the Channel to attach a per-socket fanout consumer pump and broadcast a `subscriptions` delta downward so the producing Node knows the stream has a consumer.

The Channel surface for streams is `registerStream` / `unregisterStream` / `subscribeStream(streamId, uid, ws)` / `unsubscribeStream(streamId, ws)` / `publishStreams` / `reconcileSubscriptions`. There is no Channel-level `openStream` / `closeStream`, and there is no `{ action: 'open' }` advertisement envelope — advertisement is the declarative `streams` message and subscription is the separate `streamSubscribe` / `subscriptions` flow. A Passthrough bridges binary streams automatically, mirroring subscription deltas onto its remote channel; no application code is required in the Passthrough subclass.

On the browser, the `StreamConsumer` class (in `dist/client.js`) wires a stream into a View. The View obtains it through the client's `openStream(streamId)` export, which subscribes-then-opens, and consumes the stream through `data` / `error` / `end` events. See `binary streams.md` for the browser stream API, the worked SSH-terminal flow, and the full layer-by-layer relay description.

### CachedData integration: reacting to pipeline events

Streams don't flow through the CachedData pipeline — they use a parallel binary path. But the *decision* to open or close a stream at the Node often comes from application data. A Node's `onData`-style logic or its own runtime state determines when to call `this.openStream(streamId)` and `this.closeStream(streamId)`; the framework's declarative advertisement and subscription reconciliation then carries the stream up to subscribed clients without any further application wiring.

### Stream lifecycle cleanup

Streams must be explicitly closed. The framework does not garbage-collect idle streams.

```javascript
// Node side — closes the SubstreamDuplex and re-publishes the advertisement:
this.closeStream(streamId);

// Browser side — tears down the StreamConsumer and clears the uid mapping:
channel.closeStream(streamId);
```

Closing a stream at the Node removes it from the bottom-up `streams` advertisement on the next `publishStreams()`. Every upstream layer reconciles the removal and tears down its local `SubstreamDuplex`.

### Multiplexing and the binary frame protocol

Multiple streams share one WebSocket and one IPC channel. The binary frame header:

```
[0xFE sentinel][frameType:u8][uid:u32BE][payload:N bytes]
```

UIDs are uint32 values, local to each link and owned by that link's `StreamIndex`. The same streamId has different uids on different hops. The application uses string streamIds; the framework translates at each boundary (the Worker in particular translates between a child uid namespace and the channel uid namespace).

Frame types are static members of `BinaryFrame`: `BinaryFrame.data` (`0x01`), `.open` (`0x02`), `.close` (`0x03`), `.error` (`0x04`).

`sendBinary(frameType, streamId, payload, ws)` at the Channel/Node layer resolves `streamId → uid` and builds the frame. If `ws` is omitted at the Channel, the frame broadcasts to all connected sockets (server mode) or sends on the single connection (client mode).

### Anti-patterns

| Pattern | Why it's wrong |
|---------|---------------|
| Reading or passing uids | UIDs are framework-internal; application uses string streamIds only |
| Manual relay between SubstreamDuplex instances | Framework handles relay through Workers, Passthroughs, and Channels |
| `new SubstreamDuplex()` directly | The duplex carries no identity; use `this.openStream(streamId)` at the Node, which registers it in `StreamIndex` and wires the pumps |
| Calling `openStream` / `closeStream` on a Channel | Those methods were removed; the Channel surface is `registerStream` / `subscribeStream` / `publishStreams`. Originate streams at the Node |
| Sending a `{ action: 'open' }` stream advertisement | Advertisement is the declarative `streams` message reconciled by `StreamIndex`; subscription is the separate `streamSubscribe` / `subscriptions` flow |
| CachedData for binary content | Wrong semantics (dedup, cache, replay) and wrong encoding (JSON) |
| Inline closures for browser stream event handlers | Leaks listeners across reconnections; use named handlers with `off()` |
| Caching/replaying stream data through the stream | Streams are stateless flow; use log files + RPC exports for history |
